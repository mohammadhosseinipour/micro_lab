...
const unsigned short sec816x8[] = {
        0x00, 0xFC, 0xF9, 0xF3, 0xF9, 0xFC, 0x00, 0xFF, 0xFF, 0x00, 0xF7, 0xF7, 0xF7, 0xF7, 0x00, 0xFF   // Code for char
        };
...
void sub1()
{
    ...
    while(k<16)
    {
        for(i=0;i<8;i++)
        {
            ...
            //PORTD =PORTD &(01111111);
            PORTA=(1<<i);
            if(i+k>=16)
            {
            PORTB=sec816x8[i+k-16];
            }
            else
            {
            PORTB=sec816x8[i+k];
...
{
    glcd_clear();
    glcd_circle(32,32,25);
    glcd_line(32,32,32+19*cos(6*s),32+19*sin(6*s));
    glcd_line(32,32,32+15*cos(6*m),32+15*sin(6*m));
    glcd_line(32,32,32+11*cos(30*h),32+11*sin(30*h));
}
void sub3()
{
//i used negetive so when i calculate the clock it turns clockwise
    int h=-2,m=-15,s=-37;
    while(1)
    {
        s--;
